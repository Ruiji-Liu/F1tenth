...
class RRT(Node):
    def __init__(self):
        ...
        self.x = 0
        self.y = 0
        self.lookahead_index = 100
        self.total_points = 1000
        self.rrt_lookahead = 1.0
        self.vel = 0.6
        self.steering_angle = 0
        self.rrt_steering_angle = 0
        self.KP = 1.0
        self.i = 0
        self.rrt_index = 20
        self.collision_on_path = False
        self.read_waypoints('/home/ruiji_nb/sim_ws/src/f1tenth_lab6/lab6_pkg/scripts/practice2.csv')
    def scan_callback(self, scan_msg):
        ...
        # Mark grid cell as occupied
            if 0 <= grid_x < self.grid_width and 0 <= grid_y < self.grid_height:
                ...
                # add thickness
                if grid_y <= self.grid_size/self.resolution-2 and grid_y >= 1 and grid_x <= self.grid_size/self.resolution-2 and grid_x >= 1:
                    for dy in range(-1, 2):
                        for dx in range(-1, 2):
    ...
    def pose_callback(self, pose_msg):
        ...
        if self.occupancy_grid is not None:
            # _, _, goal_index, pp_steering_angle = self.find_goal(self.waypoints, initial_x, initial_y)
            # print("goal", self.goalx, self.goaly)
            current_index = self.find_closest_index(initial_x, initial_y)
            goal_index = current_index + self.lookahead_index
            if goal_index >= self.total_points:
                goal_index = goal_index - self.total_points
            self.goalx = self.waypoints[goal_index][0]
            self.goaly = self.waypoints[goal_index][1]
            self.marker_publisher(self.goalx, self.goaly)
            # current_index = self.find_closest_index(initial_x, initial_y)
            rrt_goal_index = goal_index + self.rrt_index
            if rrt_goal_index >= self.total_points:
                rrt_goal_index = rrt_goal_index - self.total_points
            reference_path = self.reference_path(current_index, rrt_goal_index)
            ...
            if self.collision_on_path is True: 
                ...
                self.find_steering_angle(goal_index, initial_x, initial_y)
                drive_msg = AckermannDriveStamped()
                drive_msg.drive.steering_angle = self.pp_steering_angle * self.KP
        ...
        for i in range(len(self.path)):
            ...
            if self.rrt_lookahead < distance < min_dist:
                ...
                if abs(angle) < 2* np.pi / 3:
                    min_dist = distance
                    self.steering_angle = (2 * y_v) / distance**2 
        # TODO: transform goal point to vehicle frame of reference
        
        # TODO: publish drive message, don't forget to limit the steering angle.
        drive_msg = AckermannDriveStamped()
        drive_msg.drive.steering_angle = self.steering_angle * self.KP
    ...
    def check_collision(self, nearest_node, new_node):
        ...
        if new_grid_y >= 30:
            new_grid_y = 29
        # print("new_grid_y", new_grid_y)
        # if new_grid_y == 30:
        #     print(y2)
        # print("new_grid_x", new_grid_x)
    ...
    def find_path(self, tree, latest_added_node):
        ...
        return path
    def find_steering_angle(self, goal_index, initial_x, initial_y):
        distance = np.sqrt((initial_x - float(self.waypoints[goal_index][0]))**2 + (initial_y - float(self.waypoints[goal_index][1]))**2)
        y_v = -(float(self.waypoints[goal_index][0]) - initial_x) * np.sin(self.theta) + (float(self.waypoints[goal_index][1]) - initial_y) * np.cos(self.theta)
        self.pp_steering_angle = (2 * y_v) / distance**2   

...
class mpc_config:
    ...
    Qk: list = field(
        default_factory=lambda: np.diag([10.5, 10.5, 10.0, 1.0])
...
class MPC(Node):
    ...
    def __init__(self):
        ...
        self.drive_pub = self.create_publisher(
            ...
            drive_topic,
            10)
        self.trajectory_pub = self.create_publisher(MarkerArray, '/trajectory_marker', 10)
        self.waypoints = None
        ...
        self.vel = 1.2
        self.init_flag = 0
        self.read_waypoints('/home/ruiji_nb/sim_ws/src/f1tenth_lab7/mpc/scripts/new_waypoints.csv')
    ...
    def read_waypoints(self,file_path):
        ...
        with open(file_path, mode='r') as csv_file:
            ...
            for row in csv_reader:
                ...
                # temp = [row[0],row[1], 1.0, 0.0]
                waypoints.append(temp+[self.vel,0.00])
    ...
    def pose_callback(self, pose_msg):
        ...
        ref_path = self.calc_ref_trajectory(vehicle_state, self.waypoints[:, 0], self.waypoints[:, 1], self.waypoints[:, 3], self.waypoints[:, 2])
        print(ref_path)
        self.publish_path(ref_path, color=(0.0, 1.0, 0.0))
        ...
        print("steering_output", steer_output)
    ...
    def mpc_prob_init(self):
        ...
        # print("uk shape", np.shape(self.uk))
        ...
        objective_1 = cvxpy.quad_form(cvxpy.vec(self.uk), R_block)
        ...
        objective_2 = cvxpy.quad_form(cvxpy.reshape(self.xk - self.ref_traj_k, (-1, 1)), Q_block)
        # TODO: Objective part 3: Difference from one control input to the next control input weighted by Rd
        objective_3 = cvxpy.quad_form(cvxpy.vec(cvxpy.diff(self.uk, axis=1)), Rd_block)
        # --------------------------------------------------------
        ...
        xk_current = self.xk[:, :-1]
        xk_next = self.xk[:, 1:]
        constraints.append(cvxpy.reshape(xk_next, (-1,1)) == self.Ak_ @ cvxpy.reshape(xk_current, (-1, 1)) + self.Bk_ @ cvxpy.reshape(self.uk, (-1, 1)) +
        ...
        delta_steering = cvxpy.abs(cvxpy.diff(self.uk[1, :]))
        steering_constraint = delta_steering <= self.config.MAX_DSTEER * self.config.DTK
        constraints.append(steering_constraint)
        ...
        state_bounds_constraints = [self.xk[2, :] >= self.config.MIN_SPEED,self.xk[2, :] <= self.config.MAX_SPEED]
        input_bounds_constraints = [
            cvxpy.abs(self.uk[0, :]) <= self.config.MAX_ACCEL,
            self.uk[1, :] >= self.config.MIN_STEER,
            self.uk[1, :] <= self.config.MAX_STEER
        ]
        constraints += initial_state_constraint + state_bounds_constraints + input_bounds_constraints
        # -------------------------------------------------------------
    ...
    def calc_ref_trajectory(self, state, cx, cy, cyaw, sp):
        ...
        # print(np.shape(np.array([cx, cy])).T)
    ...
    def update_state(self, state, a, delta):
        ...
        return state
    def publish_path(self, ref_path, color):
        ...
        # print(np.shape(ref_path)[1])
        # for i in range(np.shape(ref_path)[1]):
        marker.points = [Point(x=ref_path[0][i], y =ref_path[1][i], z=0.0) for i in range(np.shape(ref_path)[1])]
